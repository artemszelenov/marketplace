...
export async function load({ params, locals, url }) {
  ...
  const product: Product = {
    ...
    gallery: product_record.gallery.map((file_name: string) => {
      return locals.pb_helpers.files.getFileUrlWithCorrectOrigin(
        product_record,
        file_name
      );
    }),
    image: locals.pb_helpers.files.getFileUrlWithCorrectOrigin(
      product_record,
      product_record.gallery[0]
    ),
  };
  ...
  for (const size_record of sizes_records) {
    for (const stock_item_record of stock_item_records) {
      if (stock_item_record.size_group === size_record.expand?.group.id) {
        const stock_item = stock_items.find(
          (stock_item) => stock_item_record.id === stock_item.id
        );
        const metric_value = StockItemsMetricsSchema.parse(
          size_record.expand?.metric.value
        );
...
async function getRecords(pb: PocketBase, product_slug: string) {
  const product_record = await pb.collection("products").getOne(product_slug, {
    expand: "type, color",
  });

  const stock_item_records = await pb.collection("stock_items").getFullList({
    filter: `product = "${product_record.id}"`,
    expand: "size_group",
  });

  const sizes_records = await pb.collection("sizes").getFullList({
    filter: stock_item_records
      .map(({ expand }) => `group = "${expand!.size_group.id}"`)
      .join(" || "),
    expand: "metric, group",
  });

  const product_variants_records = await pb.collection("products").getFullList({
