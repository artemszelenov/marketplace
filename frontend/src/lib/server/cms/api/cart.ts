...
export async function getCartItems(query: typeof fetch, productsIDs: string): Promise<Product[]> {
  const res = await query(`${BACKEND_URL}/api/products?where[id][in]=${productsIDs}`, {
    method: 'GET',
    credentials: 'include',
    headers: {
